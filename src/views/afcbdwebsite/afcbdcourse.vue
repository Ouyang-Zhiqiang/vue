<template>
  <div id="container" style="padding:50px">
    ...
    <el-tabs type="border-card" style="margin-top:20px">
      <el-tab-pane label="官网课程介绍">
        <el-button type="success" @click="toOpen()">新增课程介绍</el-button>

        <el-dialog title="新增课程介绍" :visible.sync="dialogFormVisible" style="width:1200px;margin:0 auto">
          <el-form :model="obj">
            <el-form-item label="课程名称" :label-width="formLabelWidth">
              <el-input v-model="obj.title" style="width:270px;float:left" />
            </el-form-item>

            <el-form-item label="课程描述" :label-width="formLabelWidth">
              <el-input v-model="obj.descs" style="width:270px;float:left" />
            </el-form-item>
        ...
        <el-dialog title="网站课程编辑" :visible.sync="dialogFormVisible2" style="width:1200px;margin:0 auto">
          <el-form :model="updateForm">
            <el-form-item label="课程名称" :label-width="formLabelWidth">
              <el-input v-model="updateForm.title" style="width:270px;float:left" />
            </el-form-item>

            <el-form-item label="课程描述" :label-width="formLabelWidth">
              <el-input v-model="updateForm.descs" style="width:270px;float:left" />
            </el-form-item>
        ...
        <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="margin-top:20px">
          ...
          <el-table-column align="center" label="课程表述" width="250">
            <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    insertimg(){
      this.$axios.post('https://www.facebodyfitness.com/hi/main?hi=24CDIAEC0WTD', this.$qs.stringify(this.obj), {headers: {'Content-Type':'application/x-www-form-urlencoded'}}).then((res)=>{
        this.$message({
            message: '恭喜你，操作成功',
            type: 'success'
          })
    ...
    teDelete(item){
        var obj={}
        obj=item
        this.$axios.post('https://www.facebodyfitness.com/hi/main?hi=24CDIAEC1TLB', this.$qs.stringify(obj), {headers: {'Content-Type':'application/x-www-form-urlencoded'}}).then((res)=>{
         this.getAllList()
         this.$message({
            message: '恭喜你，操作成功',
            type: 'success'
    ...
    updateImg(){
      this.$axios.post('https://www.facebodyfitness.com/hi/main?hi=24CDIAEC1U3E', this.$qs.stringify(this.updateForm), {headers: {'Content-Type':'application/x-www-form-urlencoded'}}).then((res)=>{
         this.getAllList()
    ...
    // },
    getAllList(){
      this.$axios.post('https://www.facebodyfitness.com/hi/main?hi=24CDIAEC0WU5', {headers: {'Content-Type':'application/x-www-form-urlencoded'}}).then((res)=>{
